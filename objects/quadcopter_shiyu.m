%% SIMPLE QUADCOPTER (from code provided by Dr. Shiyu Zhao)
...
classdef quadcopter_shiyu < quadcopter
    properties
    ...
    methods
        % CONSTRUCTOR METHOD
        function obj = quadcopter_shiyu(varargin)
...
%             R0 = OMAS_geometry.quaternionToRotationMatrix(qn0); % For validation
...
%             [obj,~,~] = obj.getAgentUpdate(varargin{1});
            ...
            [obj.localState] = obj.updateLocalState(TIME,obj.localState);

            % GET THE LATEST ROTATION MATRIX
            Rnew = reshape(obj.localState(7:15),3,3);                      % Is fixed axes to global
            ...
            obj.DATA.inputs(1:3,TIME.currentStep) = obj.localState(16:18);                        % Record the control inputs
        end
        ...
        function [out] = fcn_dynamicsAndControl(obj,in)
            ...
            R_dot=R_BG*obj.fcn_skewSymmetric(omega);
