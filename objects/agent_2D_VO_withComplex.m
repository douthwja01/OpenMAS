...
%%  CLASS METHODS
    methods 
        ...
        function obj = agent_2D_VO_withComplex(varargin)
            ...
            obj.VIRTUAL.detectionRadius = 10;                              % Overwrite agent_2D's radius  
            
            % GEOMETRIC PARAMETERS
            obj.radius = 0.5;
            obj.VIRTUAL.radius = obj.radius;                               % Simulation parameter           
            obj.GEOMETRY = obj.getObjectGeometry(obj);                     % Update the object geometry
...
%             [obj,obstacleSet,agentSet] = obj.getSensorUpdate(dt,varargin{1});              % REALISTIC INFORMATION UPDATE
            ...
            if obj.objectID == visualiseAgent && visualiseProblem == 1
                ...
                [overHandle] = obj.getObjectScene([obstacleSet;agentSet;waypointSet],overHandle);
...
%                 neighbourConditionB = norm(knownObstacles(item).position) < obj.neighbourDist;  % [CONFIRMED]
                ...
                p_b = knownObstacles(item).position(1:2,1) + p_a;          % The relative -> absolute coordinates
                v_b = knownObstacles(item).velocity(1:2,1) + v_a;          % Convert relative parameters to absolute
                g_b = knownObstacles(item).geometry;                       % Convert to p_a relative coordinates
                %g_b.vertices = g_b.vertices + [p_a;0]';
                tau_b = 0;
                
                % DEFINE THE VELOCITY OBSTACLE PROPERTIES
                if knownObstacles(item).type == OMAS_objectType.obstacle
                    % CALCULATE THE VELOCITY FOR COMPLEX VELOCITY
                    [VO_i] = obj.define2DVelocityObstacle_complex(p_a,v_a,r_a,p_b,v_b,g_b,tau_b);
                else
