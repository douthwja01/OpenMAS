...
%             obj.maxNeighbours = 10;  
    ...
    methods 
        ...
        function [headingVector,speed] = getAvoidanceCorrection(obj,dt,prefVelocity,knownObstacles,visualiseProblem)
            % This function computes the mapping of the simulation inputs
            % for the RVO library functions and returns the calculated
            % optimal velocity.
            
            % PARSE OBSTACLE TYPES
            agentSet    = knownObstacles([knownObstacles.type] == OMAS_objectType.agent);
            obstacleSet = knownObstacles([knownObstacles.type] == OMAS_objectType.obstacle);
            ...
            % BUILD THE REQUIRED AGENT STRUCTURES
            agentNeighbours = cell(numel(agentSet),1);
            agentNeighbourLength = 0;
            for item = 1:numel(agentSet)
                % NEIGHBOUR CONDITIONS
                neighbourConditionA = agentNeighbourLength < obj.maxNeighbours; 
                ...
                agentRef = struct('position',neighbourPosition,...
                                  ...
                                  'unitDir',obj.nullVelocityCheck(neighbourVelocity),...
            ...
            for item = 1:numel(obstacleSet)
                % PROCESS THE OBSTACLE'S VERTICES
                obstacleRef = obstacleSet(item);
                obstacleRef.position = obstacleRef.position + agentPosition;
                obstacleRef.velocity = obstacleRef.velocity + agentVelocity;
%                 obstacleRef.geometry.vertices = obstacleRef.geometry.vertices + [agentPosition;0]';
                obstacleNeighbours{item} = obj.createRVOobstacleVertexList(obstacleSet(item));
            end
            % REMOVE EMPTY CELLS
