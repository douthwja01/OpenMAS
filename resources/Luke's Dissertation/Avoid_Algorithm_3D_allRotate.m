function [VelA,Vel_Collide_union] = Avoid_Algorithm_3D_allRotate(PosA,PosB,VelA,VelB,Size,Vel_Collide_Prev,Vel_Start,colour,precision)
%% Avoid Algorithm
...
if VelA(1:2) == [0 0]  
    ...
    VelA = (Rx * VelA')';
    VelA = (Rz * VelA')';
    VelA = round(VelA/precision)*precision;
end

if Vel_Start(1:2) == [0 0]
    Vel_start = Vel_Start;
else    
    Vel_start = (Rz * [Vel_Start,1]');
    Vel_start = (Rx * [Vel_Start,1]')';
    Vel_start = round(Vel_start/precision)*precision;
end
...
if ismember(Vel_start(1:3),Vel_Collide_union(:,1:3),'rows') == 0    
    VelA = [Vel_Start,1];    
    velStart = 1;
elseif ismember(VelA(1:3),Vel_Collide_union(:,1:3),'rows') == 1    
    A_tilda = A(:,1:3) + VelA(:,1:3); 
