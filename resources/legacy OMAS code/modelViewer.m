...
figureProperties.positionOffset = [0;0;0];
figureProperties.R = OMAS_geometry.eulersToRotationMatrix([deg2rad(0);deg2rad(0);deg2rad(25)]);
figureProperties.triadScale = 1;
figureProperties.triad_ROffset = OMAS_geometry.eulersToRotationMatrix([pi;0;0]);
figureProperties.triadPositionOffset = [0.5;-0.5;-0.5];
...
%% ADD GLOBAL TRIAD
triadHandle = OMAS_graphics.drawTriad(figureHandle,figureProperties.triadPositionOffset,figureProperties.triad_ROffset,0.3*figureProperties.triadScale);
set(triadHandle(1,:),'lineWidth',1.2);
set(triadHandle(1,:),'lineStyle','-');
% set(triadHandle(1,:),'Color','k');