...
%   axis([xmin xmax ymin ymax])
  ...
  if isa(X,'intval')
    ...
    if isa(Y,'intval')      % interval vectors X and Y
      ...
      Y.sup = Y.sup(:)';
      if ~exist('Z','var')
          h = fill([X.inf;X.sup;X.sup;X.inf],[Y.inf;Y.inf;Y.sup;Y.sup],c);
      else
          h = fill3([X.inf;X.sup;X.sup;X.inf],[Y.inf;Y.inf;Y.sup;Y.sup],repmat(Z,4,1),c);
      end
      ...
      Y = Y(:)';
      if ~exist('Z','var')
          h = fill([X.inf;X.sup],[Y;Y],c);
%       fill([X.inf;flipud(X.sup)],[Y;flipud(Y)],c)
      else
          h = fill3([X.inf;X.sup],[Y;Y],repmat(Z,2,1),c);
      end
    end
    ...
    if ~exist('Z','var')
        h = fill([X;flipud(X)],[Y.inf;flipud(Y.sup)],c);
    else
        h = fill3([X;flipud(X)],[Y.inf;flipud(Y.sup)],repmat(Z,2,1),c);
    end
  end
