...
namespace RVO {
	Agent::Agent(RVOSimulator *sim) : maxNeighbors_(0), maxSpeed_(0.0f), neighborDist_(0.0f), radius_(0.0f), sim_(sim), timeHorizon_(0.0f), timeHorizonObst_(0.0f), id_(0), lp1(std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN()), lp2(std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN()), lp3(std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN()), lp2OpVel(std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN()) { }
	...
	void Agent::computeNewVelocity()
	{
		...
		//float lp1x, lp1y, lp2x, lp2y, lp3x, lp3y;

		//lp3x = -1000;
		//lp3y = -1000;

		sim_->outputFile << sim_->getGlobalTime() << ',' << id_ << ',' << position_.x() << ',' << position_.y() << ',';
		...
		//if (id_ == 0)
		//{
			sim_->outputFile << velocity_.x() << ',' << velocity_.y() << ',' << 10000 << ',' << lp1.x() << ',' << lp1.y() << ',' << 20000 << ',' << lp2.x() << ',' << lp2.y() << ',' << 30000 << ',' << lp3.x() << ',' << lp3.y() << ',' << lp2OpVel.x() << ',' << lp2OpVel.y() << ',';
				


			lp1 = Vector2(std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN());
			lp2 = Vector2(std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN());
			lp3 = Vector2(std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN());

			lp2OpVel = Vector2(std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN());

				
				//<< ',' << newVelocity_.x() << ',' << newVelocity_.y() << ',' << ',' << ','; 

			//outputFile << sim->getGlobalTime() << ',';

			//sim

			//std::cout << "Agent " << id_ << " " << newVelocity_ << std::endl;
		//}
	}
	...
	bool Agent::linearProgram1(const std::vector<Line> &lines, size_t lineNo, float radius, const Vector2 &optVelocity, bool directionOpt, Vector2 &result)
	{
		...
		//sim_->outputFile << 10000 << ',' << result.x() << ',' << result.y() << ',';

		lp1 = Vector2(result.x(), result.y());
	...
	size_t Agent::linearProgram2(const std::vector<Line> &lines, float radius, const Vector2 &optVelocity, bool directionOpt, Vector2 &result)
	{
		...
		//sim_->outputFile << 20000 << ',' << result.x() << ',' << result.y() << ',';

		lp2 = Vector2(result.x(), result.y());
	...
	void Agent::linearProgram3(const std::vector<Line> &lines, size_t numObstLines, size_t beginLine, float radius, Vector2 &result)
	{
		float distance = 0.0f;
		...
		for (size_t i = beginLine; i < lines.size(); ++i) {
			if (det(lines[i].direction, lines[i].point - result) > distance) {
				...
				//sim_->outputFile << 30000 << ',' << result.x() << ',' << result.y() << ',';

				lp3 = Vector2(result.x(), result.y());
