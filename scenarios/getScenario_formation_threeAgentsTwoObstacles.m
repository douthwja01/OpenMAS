function [ objectIndex ] = getScenario_formation_threeAgentsTwoObstacles(varargin)
% This function generates the three agent, two obstacle example, termed
% scenario C in the formation control/ collision avoidance study.

fprintf('[SCENARIO]\tGetting the three agent, two obstacle formation control example.\n');
...
assert(agentNumber == 3,'This scenario (scenario C) requires three input agents, specified by the "agent" attribute.');
...
% We want to build a asymmetric agent distribution, two inner agents, one
% outer agent.

% INNER AGENTS
agentScenario = scenarioBuilder(2);
[ agentConfigA ] = agentScenario.planarRing('velocities',inputConfig.velocities,...
                                                'radius',inputConfig.agentOrbit);                                         
% OUTER AGENTS 
agentScenario = scenarioBuilder(1);                                             
...
for index = 1:obstacleNumber                                 
    ...
    obstacleIndex{index}.VIRTUAL.name = sprintf('OB:%s',inputConfig.obstacles{index}.name);
