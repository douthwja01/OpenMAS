...
classdef OMAS_monteCarlo
   ...
   methods
       ...
       function [obj] = OMAS_monteCarlo(varargin)
           ...
           ind = strfind(pathString,'environment');
           ...
           % Parse the inputs ag
           [monteCarloConfig] = GetParameterOverrides(obj.GetDefaultMCconfig(),varargin);
           % Parse the config parameters against the object
           fnames = fieldnames(monteCarloConfig);
           for i = 1:numel(fnames)
               if isprop(obj,fnames{i})
                    obj.(fnames{i}) = monteCarloConfig.(fnames{i});
               end
           end
           [obj] = GetParameterOverrides(obj,monteCarloConfig);     
           
           % PARSE DEFAULT OMAS CONFIGURATION
           obj.OMAS_settings = GetParameterOverrides(obj.GetDefaultOMASConfig(),varargin); % Parse input config against the default config
           ...
           % Record the number of sessions 
           obj.sessions   = size(obj.objects,1);
           obj.outputPath = OMAS_system.GetOSPathString(obj.outputPath);   % Attempt to OS-normalise the requested paths
   ...
   methods
       ...
       function [] = evaluateAllCycles(obj)
           ...
           [highestLevelPath] = obj.GetMonteCarloDirectory(obj.outputPath);
           ...
           if obj.threadPool
               obj.GetThreadpoolInterface('CONFIRM');
           end
           ...
           for i = 1:obj.sessions 
               ...
               sessionPath = obj.GetSessionDirectory(highestLevelPath,sessionLabel);                            
               % ///////////////// COMPUTE THE CYCLES /////////////////////
               if obj.threadPool
           ...
           if obj.threadPool
               parfor i = 1:obj.sessions
                   % SESSION PATH
                   sessionPath = OMAS_system.GetOSPathString([highestLevelPath,'\',sessionList{i}]);
           ...
           % Define the path to the figure output
           studyOutputPath = OMAS_system.GetOSPathString([highestLevelPath,'\']);
           ...
           obj.GetMeanComputationTimeSeries(figureProperties,studyOutputPath,summaryData);
           % GENERATE THE FIGURES COMPARING MEAN COMPUTATIONAL TIMES TO AGENT POPULATION
           obj.GetMeanComputationTimesVsPopulation(figureProperties,studyOutputPath,summaryData);
           % GENERATE THE FIGURES COMPARING MEAN NUMBER OF COLLISIONS TO AGENT POPULATION
           obj.GetMeanCollisionsVsPopulation(figureProperties,studyOutputPath,summaryData);
           % //////////////////////////////////////////////////////////////
           
           % KILL THE POOL WHEN ALL SESSIONS/STUDIES ARE COMPLETE
           if obj.threadPool
               obj.GetThreadpoolInterface('KILL');
           end
       end
       ...
       function [summaryData] = computeSessionAnalyitics(obj,sessionPath)
           ...
           fileName = OMAS_system.GetOSPathString([sessionPath,'\','summaryData.csv']);
           ...
           for i = 1:totalSamples
               % The cycle path
               monteCarloPath = OMAS_system.GetOSPathString([sessionPath,'\',cycleDirectories{i}]);
               % IMPORT THE CYCLE SAMPLE
               [cycleSample,~] = obj.importPathData(obj.systemFiles,monteCarloPath);
           ...
           % The cycle path
           meanDataPath = OMAS_system.GetOSPathString([sessionPath,'\meanData.mat']);
           save(meanDataPath, '-struct', 'summaryData');
...
%                dlmwrite(fileName,cycleData,'delimiter',',','-append');
   ...
   methods (Static)
       ...
       function [sessionPath]    = GetSessionDirectory(outputPath,fileLabel)
           ...
           % Parse the path into a string friendly to the host OS
           sessionPath = OMAS_system.GetOSPathString([outputPath,fileString]);
           % Create output directory
           assert(mkdir(sessionPath) == 1,'Unable to create session directory.');
       end
       % BUILD THE RELATIVE MONTE-CARLO DIRECTORY
       function [monteCarloPath] = GetMonteCarloDirectory(outputPath,fileLabel)
           % This function build the monte-carlo data output directory. The
           % naming of the file is of the form:
       ...
       function [poolObject]    = GetThreadpoolInterface(conf)
       ...
       function [defaultConfig] = GetDefaultMCconfig()
           ...
           % Get the path to the user-space
           [~, userdir] = system('echo %USERPROFILE%');                    % Get desktop path
                      
           % DEFAULT (GENERIC) MONTE-CARLO SETTINGS
           defaultConfig = struct();
           defaultConfig.outputPath = OMAS_system.GetOSPathString([userdir,'\desktop\US18_data']);  
           % DEFAULT MONTE-CARLO PARAMETERS 
