...
% CONSTANTS
pathString = mfilename('fullpath'); % Get the system paths
ind = strfind(pathString,'env');
TEST.tempDir = pathString(1:(ind-1));      
TEST.tempDir = OMAS_system.GetOSPathString(TEST.tempDir);

TEST.objectsPath  = '\objects';
TEST.objectNumber = 2;
TEST.phase        = 'DEBUG';
TEST.summaryFile  = OMAS_system.GetOSPathString([pwd,'\ObjectDebugReport.csv']);
isIssue = 0;
